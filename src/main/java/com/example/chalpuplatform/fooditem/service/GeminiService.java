    ...
    private Map<String, Object> buildGeminiRequest(String base64Image, String mimeType) {
        ...
        String prompt = """
            ...
            **중요 지침:**
            ...
            2. 메뉴명은 정확히 이미지에 나온 그대로 작성하세요. 메뉴 명이 30자를 초과할 경우, 해당 메뉴는 건너뛰세요.
            3. 가격은 원(₩) 기호나 콤마 없이 숫자만 입력 (예: 8000)
            4. 카테고리는 다음 중 선택: 메인요리, 사이드, 음료, 디저트, 전채요리, 스프, 샐러드, 기타
            5. 메뉴는 같은데 사이즈가 다른 경우 메뉴 이름은 같게 하되 메뉴 설명(description)에 사이즈를 적어주세요.
            6. 세트메뉴나 콤보는 개별 항목으로 분리하지 말고 하나로 처리
            7. 읽기 어려운 부분은 추측하지 말고 건너뛰세요
            8. JSON 외의 다른 텍스트는 절대 포함하지 마세요
            """;
